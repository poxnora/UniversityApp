...
public class ZajeciaZaliczeniaEdycjaController implements Initializable , ILoadData {
    ...
    void zatwierdz(ActionEvent event) {
        ...
        try {
            ...
            Nawigator.changeSceneLoadData(Nawigator.Resource.PROWADZACY_ZAJECIA_ZALICZENIA_PODGLAD, prevViewData, event);
    ...
    void zajecia(ActionEvent event) {
        try {
            Nawigator.changeScene(Nawigator.Resource.PROWADZACY_ZAJECIA_ZALICZENIA_EDYCJA, event);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
